...
META_LEAK_PATTERNS = [
    re.compile(r"^从(?:岗位沟通|沟通|JD|岗位信息|您发的消息|目前情况|背景)来看[，,:]?\s*", re.I),
    re.compile(r"^(?:针对|关于)您(?:提?出的)?问题[，,:]?\s*", re.I),
    re.compile(r"^根据您(?:提?出的)?(?:要求|信息|问题)[，,:]?\s*", re.I),
    re.compile(r"^作为(?:一名)?(?:候选人|求职者|应届生|在校生|新人)[，,:]?\s*", re.I),
    re.compile(r"^结合(?:您这边的)?(?:岗位要求|JD|背景)[，,:]?\s*", re.I),
]

# 真实人类会话微语用变奏池（打破 LLM 确定性贪婪输出与模版复读，注入真人自然随机性）
DYNAMIC_TONE_VARIANTS = (
    "【本次口语变奏要求】：语气自然随和，用生活化口语从容作答或反问，句式结构自如，严禁直接照抄提示词示例原句，用自己的日常词汇表述。",
    "【本次口语变奏要求】：极其干脆利索，短促从容，问答直接收敛闭环，严禁套话八股与原句复读，口语节奏紧凑自然。",
    "【本次口语变奏要求】：平实真诚，略带生活化探究感，换用不同的代词、语气助词或句首词，用自然的生活化短语表达。",
    "【本次口语变奏要求】：直截了当，句式轻快自然，注重真实口语的即兴感，绝不使用死板的千人一面句式。",
    "【本次口语变奏要求】：从容坦诚，以第一人称口语直接说事实与直观想法，词汇搭配自然随机，杜绝任何模板背书感。",
)
...
class AIReplyEngine:
    ...
    def build_agent_prompt(self, conv: dict) -> str:
        ...
        if not history_section:
            history_section = "\n【本场会话状态：首轮沟通 / 刚刚建立连接（双方此前尚无正式对话历史）】\n"
        ...
        return (
            ...
            f"   - 【首句打招呼与简单问候规范】：若 HR 仅发送“你好/您好/在吗/你好？”等常规打招呼问候，且上方无历史会话记录（首轮开场），请像真人一样礼貌自然应答（如“您好！看到咱们这个岗位很感兴趣，方便简单请教下目前团队的业务推进方向吗？”或“您好！在看消息的，咱们这个岗位方便进一步聊聊吗？”）。【绝对严禁】过度防卫或无端自证（严禁在对方未怀疑时莫名其妙说“是本人在看消息”；严禁在对方未询问常驻地时无脑背诵常驻城市）；\n"
            f"1. 【严禁暴露思考视角与元分析语言（最高红线）】：\n"
            f"   - 严禁出现任何“从...来看”、“从岗位沟通来看”、“从JD来看”、“从公司信息看”、“根据您的提问/发言”、“针对您的问题”、“作为候选人/求职者/应届生/新人”等暴露自己在做局外分析的元语言！\n"
            f"   - 永远不要向对方阐述“我是从什么角度推导出来的”，真人只会直接说事实、直观理解或结论；\n"
            f"   - 严禁出现“感谢您的详细介绍”、“期待进一步沟通”、“希望有机会加入”、“非常荣幸”、“祝好”等任何客服式/三段式礼貌八股废话！开门见山直接说事；（注：若对方询问生日表达入职蛋糕关怀，可真诚平实回应“太贴心了谢谢公司关怀！”，严禁客服大段八股套话）\n"
            ...
            f"   - 【复合问题必须完整回应】：若 HR 在一条消息中提出了复合问题（例如既询问个人情况/人情关怀/生日/到岗时间/业务问题，又提出索要微信/简历/电话等），【绝对严禁】仅机械复读一句动作通知！必须【先真诚、得体、就事论事地回答 HR 提出的前半部分问题】（如对方询问生日入职蛋糕，结合候选人真实画像告知是8月份生日，并对入职蛋糕关怀礼貌致谢，例如“我是8月份的生日，太贴心了谢谢公司关怀！”；询问到岗则告知到岗时间），【然后再自然告知已在平台发起相应动作】（如“已在平台向您发起交换微信申请，期待进一步交流”）；\n"
            ...
            f"    - {random.choice(DYNAMIC_TONE_VARIANTS)}\n\n"
            f"请输出纯 JSON 格式：\n"
    ...
    def _llm_call_once(self, prompt: str) -> Optional[dict]:
        ...
        try:
            ...
            payload = {
                "model": model,
                "messages": [
                    {"role": "system", "content": "你是一名真实的人类求职者（20+岁年轻职场人），正在BOSS直聘手机端与HR实时沟通。以问答为主，就事论事，回答简明利索（15-40字自然口语），平实客观真诚，具有真实的语言随机性与多样性，绝不死板复读。严格输出纯JSON。"},
