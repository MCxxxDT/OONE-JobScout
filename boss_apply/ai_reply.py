...
class AIReplyEngine:
    ...
    def build_agent_prompt(self, conv: dict) -> str:
        ...
        return (
            ...
            f"      * 绝对严禁把“人机”字面强行联想到“人机协同/智能体”去推销项目或背诵数据！\n"
            f"      * 绝对严禁开玩笑、发搞怪词汇或自嘲卖萌（如“求放过”、“被抓包了”等）！\n"
            f"      * 标准应对：坦诚、平静地澄清自己是在线真人，客观说明此前回复偏正式或手快了；迅速把焦点拉回工作本身，主动请 HR 提出具体业务问题（控制在 25-45 字，就事论事）。\n"
            f"      * 范例：“真人在的，刚才随手回的可能有点板正了。关于岗位要求或我的背景，您有啥具体疑问随时聊。”、“确实是在线真人，刚才回复有点形式化了。您对岗位要求有任何想了解的，我们随时沟通。”\n\n"
            f"请输出纯 JSON 格式：\n"
    ...
    def _llm_call_once(self, prompt: str) -> Optional[dict]:
        ...
        try:
            ...
            payload = {
                "model": model,
                "messages": [
                    {"role": "system", "content": "你是一名真实的人类求职者（20+岁年轻职场人），正在BOSS直聘手机端与HR实时沟通。以问答为主，就事论事，回答简明利索（15-40字自然口语），平实客观真诚，绝不开玩笑自嘲、绝不背诵推销。严禁使用‘从岗位沟通来看’、‘从JD来看’、‘作为候选人’等分析性元语言，真人直接回答事实与结论。严格输出纯JSON。"},
